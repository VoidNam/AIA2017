...
Mat Aia5::calcCompLogL(vector<struct comp*>& model, Mat& features){
	...
	Mat LogL = Mat::zeros(compNum, featureNum, CV_32FC1);

	for(int i = 0; i < compNum; i++){
		Mat mean = model[i]->mean;
		Mat covar = model[i]->covar;
		double det = determinant(covar);
		
		for(int j = 0; j < featureNum; j++){
			Mat MeanD = mean - features.col(j);
			Mat tmp = MeanD.t() * covar.inv() * MeanD;
			LogL.at<float>(i, j) = -0.5 * featureDim * log(2 * M_PI) - 0.5 * log(abs(det)) - 0.5 * sum(tmp).val[0];//0.5 * tmp.at<float>(0, 0);
		}
	}
...
Mat Aia5::calcMixtureLogL(vector<struct comp*>& model, Mat& features){
	...
	Mat mixLogL = Mat::zeros(1, featureNum, CV_32FC1);

	for(int i = 0; i < featureNum; i++){
		double p = 0;
		double logC = log(model[0]->weight) + LogL.at<float>(0,0);
		double logC_tmp = 0;

		//find the maximum alpha times p
		for(int j = 0; j < compNum; j++){
			logC_tmp = log(model[j]->weight) + LogL.at<float>(j,i);
				if(logC_tmp > logC)
					logC = logC_tmp;
		}

		for(int j = 0; j < compNum; j++){
			p += exp(log(model[j]->weight) + LogL.at<float>(j,i) - logC);
		}

		mixLogL.at<float>(0,i) = log(p) + logC;
	}
...
Mat Aia5::gmmEStep(vector<struct comp*>& model, Mat& features){
	...
	Mat posterior = Mat::zeros(compNum, featureNum, CV_32FC1);

	for(int i = 0; i < compNum; i++){
		double alpha = model[i]->weight;
		for(int j = 0; j < featureNum; j++){
			posterior.at<float>(i,j) = exp(log(alpha) + LogL.at<float>(i,j) - mixLogL.at<float>(0,j)); 
		}
	}
...
void Aia5::gmmMStep(vector<struct comp*>& model, Mat& features, Mat& posterior){
   // To Do !!!
	...
	for(int i = 0; i < compNum; i++){
		Mat mean = Mat::zeros(model[i]->mean.rows, model[i]->mean.cols, CV_32FC1);
		Mat covar = Mat::zeros(model[i]->covar.rows, model[i]->covar.cols, CV_32FC1);
		double Nj = sum(posterior.row(i)).val[0];

		for(int j = 0; j < featureNum; j++){
			mean += features.col(j) * posterior.at<float>(i,j);

			Mat tmp = features.col(j) - model[i]->mean;
			covar += tmp * tmp.t() * posterior.at<float>(i,j);
		}

		//Updates the given model
		model[i]->mean = mean / Nj;
		model[i]->weight = Nj / featureNum;
		model[i]->covar = covar / Nj;
	}			
}
...
void Aia5::run(void){
	...
	cout << endl << "Hello Ronny. Because we run under Windows we change the path a little bit." << endl;
	cout << "If you need to run this code I think maybe you need to comment out line 172, 211, 519, 538 and use the original one. :)" << endl << endl;;
	...
	cout << "msize : " << featProjection[0].mean << endl;
	cout << "timgDB: " << trainImgDB.size() << endl;
    ...
    for(int c_true=0; c_true<10; c_true++){
		...
		for(int c_est=0; c_est<10; c_est++){
			...
			for(int i=0; i<fea.cols; i++){
				if ( ( maxMixLogL.at<float>(0,i) < mixLogL.at<float>(0,i) ) || (c_est == 0) ){
...
void Aia5::readImageDatabase(string dataPath, vector<Mat>& db){
    ...
    for(int c=0; c<10; c++){
		...
		sprintf(curDir, "%s%c%i", dataPath.c_str(), delim, c);
		...
		if ( (s.st_mode & S_IFMT ) == S_IFDIR ){
			...
			while(entry = readdir(pDIR)){
				...
				Mat img = imread((curDir + string("/") + string(entry->d_name)).c_str(), 0);
